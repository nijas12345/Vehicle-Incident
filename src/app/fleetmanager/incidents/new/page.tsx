...
import { INCIDENT_TYPES, SEVERITY_LEVELS } from '../../../../../constants/incidents';
...
export default function NewIncidentPage() {
  ...
  const nonFleetUsers = users.filter((user: User) => user.role !== 'FleetManager');
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const finalData = { ...parsed.data, occurredAt: new Date(parsed.data.occurredAt) };
  ...
  if (carsLoading || usersLoading)  return (
  ...
  if (carsError || usersError) return <div className="p-6 text-red-600">Failed to load data</div>;
  ...
  <form onSubmit={handleSubmit} className="space-y-4">
    <input name="title" placeholder="Incident Title" className="border p-2 w-full rounded" />
    ...
    <select name="reportedById" className="border bg-black p-2 w-full rounded">
    ...
    <input name="location" placeholder="Location" className="border p-2 w-full rounded" />

    {/* Multiple image upload */}
    <input type="file" accept="image/*" multiple onChange={handleFileChange} className="border p-2 w-full rounded" />
    {previewUrls.length > 0 && (
      <div className="mt-2 flex gap-2 flex-wrap">
        {previewUrls.map((url, index) => (
          <div key={index} className="relative">
            <img src={url} alt={`Preview ${index}`} className="w-32 h-32 object-cover rounded" />
            <button
              type="button"
