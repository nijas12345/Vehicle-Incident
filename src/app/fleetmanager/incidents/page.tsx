...
import { IncidentTableRow } from "../../../../constants/interface";
          ...
          <tbody>
            {paginatedData?.map((incident: IncidentTableRow) => (
              <tr key={incident.id} className="bg-white">
                ...
                <td className="p-3 text-gray-700">{incident.assignedTo?.name || "N/A"}</td>
        ...
        <div className="flex justify-center mt-4 gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
            <button
              ...
              className={`px-3 py-1 rounded ${currentPage === page ? "bg-blue-600 text-white" : "bg-gray-800 hover:bg-gray-900"}`}
            >
              {page}
            </button>
          ))}
